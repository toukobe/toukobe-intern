...
export default function EarlyFormPage() {
  const router = useRouter();
  const isMobile = useIsMobile();
  const [form, setForm] = useState({ company_name: '', contact_name: '', phone: '', legal_email: '', billing_email: '', account_email: '', notes: '' });
  const [loading, setLoading] = useState(false);
  const [done, setDone] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    document.title = '早期申し込み | トウコべインターン';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/form-submit', {
        ...
        body: JSON.stringify({ form_type: 'early', ...form }),
      });
      if (!res.ok) {
    ...
    <div style={{ minHeight: '100vh', background: '#FFF6EE', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '24px 16px' : '48px 24px', fontFamily: FF }}>
      ...
      <div style={{ width: '100%', maxWidth: 560, background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,.10)', padding: isMobile ? '32px 20px' : '48px 44px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <div style={{ display: 'inline-block', background: '#FFF1E8', border: '1px solid rgba(242,98,12,.3)', borderRadius: 999, padding: '4px 14px', fontSize: 12, fontWeight: 700, color: '#F2620C', marginBottom: 12 }}>早期申し込み</div>
        ...
        {done ? (
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              ...
              <p style={S.hint}>担当者よりお電話でご連絡する場合がございます。</p>
            </div>
