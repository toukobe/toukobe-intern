  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/form-submit', {
        ...
        body: JSON.stringify({ form_type: 'material', ...form }),
      });
    ...
    <div style={{ minHeight: '100vh', background: '#FFF6EE', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '24px 16px' : '48px 24px', fontFamily: FF }}>
      ...
      <div style={{ width: '100%', maxWidth: 520, background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,.10)', padding: isMobile ? '32px 20px' : '48px 44px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <h1 style={{ fontWeight: 900, fontSize: isMobile ? 20 : 24, margin: '0 0 8px', color: '#1C1813' }}>資料請求フォーム</h1>
        ...
        {done ? (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.8, margin: '0 0 28px' }}>資料請求を受け付けました。<br />担当者より1営業日以内にご連絡いたします。</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              ...
              <input type="email" style={S.input} value={form.legal_email} onChange={e => set('legal_email', e.target.value)} onFocus={focus} onBlur={blur} placeholder="contact@example.com" required />
              <p style={S.hint}>資料および担当者からの連絡をお送りします。</p>
            </div>

            <div>
              <label style={S.label}>どこでこのサービスを知りましたか？<span style={S.req}>*</span></label>
              <select
            ...
            <div>
              ...
              <textarea
                ...
                onFocus={focus}
                onBlur={blur}
                placeholder="ご質問・ご要望などあればお知らせください。"
              />
            </div>
