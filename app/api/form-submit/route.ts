...
<table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:24px">
  ...
  ${row('その他ご要望', notes)}
    ...
    <h2 style="font-size:20px;margin:0 0 8px">お申し込みありがとうございます</h2>
    <p style="font-size:14px;color:#57514A;line-height:1.8;margin:0 0 24px">
      ${contact_name ? `${contact_name} 様、` : ''}以下の内容でお申し込みを受け付けました。<br>
      担当者より<strong>1営業日以内</strong>にご連絡いたします。
    </p>
    <div style="background:#FBF8F4;border-radius:10px;padding:16px 20px;margin-bottom:24px">
      <p style="font-size:13px;font-weight:700;color:#1C1813;margin:0 0 12px">【お申し込み内容】${formLabel}</p>
      ${answerTable}
    </div>
    ...
    const sends = [
      resend.emails.send({
        from: `トウコべインターン <${FROM}>`,
        to: ADMIN_EMAIL,
        subject: `【${formLabel}】${company_name} より申し込みがありました`,
        html: adminHtml,
      }),
    ];
