...
最終更新: 2026-07-24（画像アップロード用ストレージRLSポリシーを追加）

---

## 🔴 要対応: 画像アップロードのストレージRLS（2026-07-24）

企業のロゴ・背景画像・求人カバーのアップロードが失敗する場合、`storage.objects` に認証ユーザー向けの書き込みポリシーが無い/不適切なことが原因の可能性が高い（読み取りはバケットpublicで可、書き込みだけRLS拒否）。

- **`sql/2026-07-24_storage_policies.sql` を Supabase → SQL Editor で実行**（`company-logos`・`job-covers` へ認証ユーザーがアップロード/差し替え/削除できるようにする。何度実行しても安全）。
- 実行後もまだ失敗する場合は、アップロード時のトーストに実エラー文言を出すようにしたので、その文言を確認する。
...
- **`sql/2026-07-24_site_mode.sql` を Supabase → SQL Editor で実行**（`site_settings` テーブル作成＋初期値を「公開前(prelaunch)」で投入。何度実行しても安全）。※実行済み
- **`sql/2026-07-24_preview_key.sql` を Supabase → SQL Editor で実行**（プレビュー共有リンク用の `preview_key` 列を追加＋ランダムキー発行）。これを実行すると、管理者ページ「サイト状態」タブにプレビューリンクが表示され、ログイン不要で相手に本番画面を見せられる。未実行でもサイトモード切替自体は動作する。
